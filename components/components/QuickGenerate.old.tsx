...
export default function QuickGenerate({
    ...
    const updatedProducts = await Promise.all(
      generatedData.products.map(async (product) => {
        try {
          ...
          console.error(`[generateProductImages] error for ${product.id}:`, error);
          return product;
        }
      })
    );

    setGeneratedData({ ...generatedData, products: updatedProducts });
    setIsGeneratingImages(false);
  };

  const regenerateProductImage = async (productId: string) => {
    if (!generatedData) return;
    ...
    try {
      ...
      const updatedProducts = generatedData.products.map((p) =>
        p.id === productId ? { ...p, imageUrl: url } : p
      );

      setGeneratedData({ ...generatedData, products: updatedProducts });
    } catch (error) {
      console.error('[regenerateProductImage] error:', error);
    }
  };
  ...
  const getSiteHTML = () => {
    ...
    const safeSiteName = escapeHtml(data.name);
    const safeDescription = escapeHtml(data.description);
    const safeBackgroundUrl = backgroundUrl ? escapeHtml(backgroundUrl) : '';
    ...
    // Неоновый оверлей (включается, если в ТЗ есть неон/RGB/подсветка)
    const heroNeonOverlay = hasNeon
      ? `<div class="hero-neon"></div>`
      : '';

    // Фоновое изображение (объект поверх градиента)
    const heroBackgroundImage = safeBackgroundUrl
      ? `<div class="hero-bg" style="background-image:url('${safeBackgroundUrl}')"></div>`
      : '';
